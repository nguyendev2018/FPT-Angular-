...
export class MessagesService {
  ...
  selectedUser = new BehaviorSubject<string>('myself@angular.dev');

  constructor() {}

  handleChangeSelectedUSer = (user: string) => {
    this.selectedUser.next(user);
  };

  getMessages = (option: string) => {
    return this.messages.filter(
      (mess) => mess.to === this.selectedUser.value && mess.folder === option
    );
  };

  getDetailMessage = (id: string) => {
    return this.messages.find((item) => item._id === id);
  };
}

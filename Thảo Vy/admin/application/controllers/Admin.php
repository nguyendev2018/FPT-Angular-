...
class Admin extends BaseController
{
    ...
    {   
            ...
            $process = 'Danh sách người dùng';
            $processFunction = 'Admin/userListing';
            $this->logrecord($process,$processFunction);

            $this->global['pageTitle'] = 'COUPON: Danh sách người dùng';
    ...
    function addNew()
    {
            ...
            $this->global['pageTitle'] = 'COUPON : Thêm người dùng';
    ...
    function addNewUser()
    {
            ...
            else
            {
                ...
                if($result > 0)
                {
                    $process = 'Thêm người dùng';
                    $processFunction = 'Admin/addNewUser';
                    $this->logrecord($process,$processFunction);

                    $this->session->set_flashdata('success', 'Người dùng đã được tạo thành công');
                }
                else
                {
                    $this->session->set_flashdata('error', 'Tạo người dùng không thành công');
                }
    ...
    function editUser()
    {
            ...
            else
            {
                ...
                {
                    $this->session->set_flashdata('error', 'Cập nhật người dùng không thành công');
                }
    ...
    function logHistory($userId = NULL)
    {
            ...
            $this->global['pageTitle'] = 'COUPON : Lịch sử đăng nhập của người dùng';
    ...
    function backupLogTable()
    {
        ...
        if($backup)
        {
            $this->session->set_flashdata('success', 'Sao lưu và dọn dẹp bảng thành công');
            redirect('log-history');
        }
        else
        {
            $this->session->set_flashdata('error', 'Sao lưu và dọn dẹp bảng không thành công');
            redirect('log-history');
        }
    }
    ...
    function logHistoryBackup()
    {
            ...
            $process = 'Xem nhật ký sao lưu';
            $processFunction = 'Admin/logHistoryBackup';
            $this->logrecord($process,$processFunction);

            $this->global['pageTitle'] = 'COUPON : Lịch sử đăng nhập sao lưu của người dùngi';
    ...
    function backupLogTableDelete()
    {
        ...
        if($backup)
        {
            $this->session->set_flashdata('success', 'Dọn dẹp bảng thành công');
            redirect('log-history-backup');
        }
        else
        {
            $this->session->set_flashdata('error', 'Dọn dẹp bảng không thành công');
            redirect('log-history-backup');
        }
    }
    ...
    {       
            ...
            $process = 'Tải lên nhật ký sao lưu';
            $processFunction = 'Admin/logHistoryUpload';
            $this->logrecord($process,$processFunction);

            $this->global['pageTitle'] = 'COUPON : Tải lên nhật ký người dùng';
    ...
    function logHistoryUploadFile()
    {
        ...
        else if ($optioninput != '0' && $_FILES['filebackup']['name'] == '')
        {
            ...
            {
                $this->session->set_flashdata('error', 'Cài đặt sao lưu không thành công');
                redirect('log-history-upload');
            }
            else
            {
                $this->session->set_flashdata('success', 'Cài đặt sao lưu thành công');
                redirect('log-history-upload');
            }
    }
